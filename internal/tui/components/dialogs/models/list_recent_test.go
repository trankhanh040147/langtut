...
func TestModelList_RecentlyUsedSectionAndPrunesInvalid(t *testing.T) {
	...
	confPath := filepath.Join(cfgDir, "langtut", "langtut.json")
	...
	dataConfDir := filepath.Join(dataDir, "langtut")
	...
	origConfPath := filepath.Join(cfgDir, "langtut", "langtut.json")
	...
	dataConf := filepath.Join(dataDir, "langtut", "langtut.json")
