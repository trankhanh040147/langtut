	...
	providerOnce.Do(func() {
		...
		wg.Go(func() {
			...
			if err != nil {
				...
				errs = append(errs, fmt.Errorf("Crush was unable to fetch an updated list of providers from %s. Consider setting CRUSH_DISABLE_PROVIDER_AUTO_UPDATE=1 to use the embedded providers bundled at the time of this Langtut release. You can also update providers manually. For more info see langtut update-providers --help.\n\nCause: %w", catwalkURL, providerErr)) //nolint:staticcheck
				return
			}
